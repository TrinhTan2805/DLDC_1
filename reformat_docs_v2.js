...
// Replace the main title if it exists, or add it
if (data.includes('###### Thông tin chi tiết')) {
    // We will rebuild the content from scratch to ensure correct numbering
}

const sections = [
    { title: 'Hồ sơ đăng ký khai sinh', code: 'KS', slug: 'khaisinh', img_prefix: 'mauthuthapCSDL_img_' },
    { title: 'Hồ sơ đăng ký kết hôn', code: 'KH', slug: 'kethon', img_prefix: 'mauthuthapCSDL_kethon_img_' },
    { title: 'Hồ sơ cấp GĐKN kết hôn', code: 'GDKN', slug: 'gdknkethon', img_prefix: 'mauthuthapCSDL_gdknkethon_img_' },
    { title: 'Hồ sơ đăng ký khai tử', code: 'KT', slug: 'khaitu', img_prefix: 'mauthuthapCSDL_khaitu_img_' },
    { title: 'Hồ sơ DK nhận cha, mẹ, con', code: 'NC', slug: 'nhancha', img_prefix: 'mauthuthapCSDL_nhancha_img_' },
    { title: 'Hồ sơ đăng ký nuôi con nuôi', code: 'NN', slug: 'nuoicnuoi', img_prefix: 'mauthuthapCSDL_nuoicnuoi_img_' },
    { title: 'Hồ sơ đăng ký giám hộ', code: 'GH', slug: 'giamho', img_prefix: 'mauthuthapCSDL_giamho_img_' },
    { title: 'Hồ sơ DK chấm dứt giám hộ', code: 'CDGH', slug: 'chamdutgiamho', img_prefix: 'mauthuthapCSDL_chamdutgiamho_img_' },
    { title: 'Hồ sơ DK thay đổi TT hộ tịch...', code: 'TDHT', slug: 'thaydoihotich', img_prefix: 'mauthuthapCSDL_thaydoihotich_img_' },
    { title: 'Hồ sơ đăng ký kiểm sát việc giám hộ', code: 'KSGH', slug: 'kiemsatgiamho', img_prefix: 'mauthuthapCSDL_kiemsatgiamho_img_' },
    { title: 'Hồ sơ đăng ký giám sát việc giám hộ', code: 'GSGH', slug: 'giamsatgiamho', img_prefix: 'mauthuthapCSDL_giamsatgiamho_img_' },
    { title: 'Hồ sơ ly hôn/hủy kết hôn ở nước ngoài', code: 'LH', slug: 'lyhon', img_prefix: 'mauthuthapCSDL_lyhon_img_' }
];
...
let imgTotalCount = 23; // Start from 24 as in the first image seen

for (let i = 0; i < lines.length; i++) {
    let line = lines[i].trim();
    if (!line && lines[i - 1] === '') continue; // Skip multiple empty lines

    // Check for new major section (Hồ sơ...)
    let foundSection = sections.findIndex(s => line.includes(s.title) && lines[i].startsWith('######'));
    if (foundSection !== -1) {
    ...
    // Regular lines
    if (line.startsWith('|')) {
        // Ensure STT in tables if missing
        let cells = line.split('|').map(c => c.trim());
        if (cells.length >= 4 && cells[1] !== '' && !isNaN(cells[1])) {
            // Already has STT
        } else if (cells.length >= 4 && cells[1] === '' && cells[2] !== '') {
            // Missing STT?
        }
        newLines.push(line);
    } else if (line !== '') {
