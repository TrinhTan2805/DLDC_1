...
const rawPath = 'd:/tuphap/khodldc/dldc_1/raw_mapped_list.txt';
...
for (const line of lines) {
    if (line.toLowerCase().startsWith('hệ thống gửi') || line.toLowerCase().startsWith('==>')) continue;
    ...
    if (match && match[1]) {
        ...
        if (!seen.has(name.toLowerCase())) {
            ...
            datasets.push({ name: name, code: screenCode });
        }
    } else {
...
for (const db of datasets) {
    ...
    md += `#### ${sectionNum}. ${db.code} Màn hình thông báo lỗi tiếp nhận dữ liệu ${db.name}
- **Mã chức năng:** DC1-TT-QT-001
- **Mã màn hình:** ${db.code}

Màn hình

![Cài đặt dịch vụ (${db.code})](./images/thuthap/MH02_P04_thongbaoloi.png)
...
console.log("Successfully injected exhaustive mapped dataset list into phantichtong.md.");
