...
import { GuardianshipCertRecord } from '../GuardianshipCertModal';

interface GuardianshipTableProps {
  records: GuardianshipCertRecord[];
  currentPage: number;
  setCurrentPage: (page: number) => void;
  itemsPerPage: number;
  setItemsPerPage: (items: number) => void;
  totalRecords: number;
  onViewRecord: (record: GuardianshipCertRecord) => void;
}
        ...
        <table className="w-full border-collapse collection-table" style={{ fontSize: '16px' }}>
          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Mã hồ sơ</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Người được giám hộ</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Người giám hộ</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Loại giám hộ</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Loại đăng ký</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Tình trạng</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((record, index) => (
              <tr key={record.id} className="hover:bg-slate-50 transition-all group">
                ...
                <td className="px-4 py-3 text-center font-semibold text-slate-900 font-mono">{record.recordCode}</td>
                <td className="px-4 py-3 text-center font-semibold text-blue-700">{record.wardName}</td>
                <td className="px-4 py-3 text-center text-slate-900 font-medium">{record.guardianName}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium">{record.guardianshipType}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium">{record.registrationType}</td>
                <td className="px-4 py-3 text-center">
                   <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full font-bold uppercase tracking-wider border shadow-sm whitespace-nowrap ${record.guardianshipStatus === 'Đang giám hộ' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-slate-50 text-slate-600 border-slate-100'}`} style={{ fontSize: '12px' }}>
