...
import { CivilRegistryChangeRecord } from '../CivilRegistryChangeModal';

interface CivilRegistryChangeTableProps {
  records: CivilRegistryChangeRecord[];
  currentPage: number;
  setCurrentPage: (page: number) => void;
  itemsPerPage: number;
  setItemsPerPage: (items: number) => void;
  totalRecords: number;
  onViewRecord: (record: CivilRegistryChangeRecord) => void;
}

export function CivilRegistryChangeTable({
  records,
  currentPage,
  setCurrentPage,
  itemsPerPage,
        ...
        <table className="w-full border-collapse collection-table" style={{ fontSize: '16px' }}>
          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Người được thay đổi</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">SĐD cá nhân</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Người đi đăng ký</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Quan hệ</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Số GTT hộ tịch</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((record, index) => (
              <tr key={record.id} className="hover:bg-slate-50 transition-all group">
                ...
                <td className="px-4 py-3 text-center font-semibold text-blue-700">{record.changedPersonName}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium font-mono">{record.personalId}</td>
                <td className="px-4 py-3 text-center text-slate-900 font-medium">{record.registrantName}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium">{record.relationship}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium font-mono">{record.civilDocNumber}</td>
