...
interface NationalityInfoTableProps {
  records: NationalityRecord[];
  currentPage: number;
  setCurrentPage: (page: number) => void;
  itemsPerPage: number;
  setItemsPerPage: (items: number) => void;
  totalRecords: number;
  onViewRecord: (record: NationalityRecord) => void;
  colNameLabel?: string;
  colTypeLabel?: string;
...
export function NationalityInfoTable({
  records,
  currentPage,
  setCurrentPage,
  itemsPerPage,
  ...
  colTypeLabel = 'Quốc tịch cũ / mới',
  colNumberLabel = 'Số quyết định',
  colDateLabel = 'Ngày cấp',
}: NationalityInfoTableProps) {
        ...
        <table className="w-full border-collapse collection-table" style={{ fontSize: '16px' }}>
          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">{colDateLabel}</th>
            ...
            {records.map((record, index) => (
              <tr key={record.id} className="hover:bg-slate-50 transition-all group">
                ...
                <td className="px-4 py-3 text-center text-slate-600 font-medium">
                  {record.oldNationality || record.newNationality || record.gender || '-'}
                </td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium font-mono">{record.decisionNo}</td>
