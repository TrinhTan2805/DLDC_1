...
interface BirthCertTableProps {
  ...
  totalRecords: number;
  onViewRecord: (record: BirthRecord) => void;
}

export function BirthCertTable({
  records,
  currentPage,
  setCurrentPage,
  itemsPerPage,
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((record, index) => (
              <tr key={record.id} className="hover:bg-slate-50 transition-all group">
                ...
                <td className="px-4 py-3 text-center text-slate-600 font-medium font-mono">{record.registrationNumber || '-'}</td>
