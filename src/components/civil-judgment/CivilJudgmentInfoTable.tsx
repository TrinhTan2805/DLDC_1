...
export interface CivilJudgmentRecord {
  id: string;
  name: string;
  type?: string;
  number: string;
...
interface CivilJudgmentInfoTableProps {
  records: CivilJudgmentRecord[];
  currentPage: number;
  setCurrentPage: (page: number) => void;
  itemsPerPage: number;
  setItemsPerPage: (items: number) => void;
  totalRecords: number;
  onViewRecord: (record: CivilJudgmentRecord) => void;
  colNameLabel?: string;
  colTypeLabel?: string;
...
export function CivilJudgmentInfoTable({
  records,
  currentPage,
  setCurrentPage,
  itemsPerPage,
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((record, index) => (
              <tr key={record.id} className="hover:bg-slate-50 transition-all group">
                ...
                <td className="px-4 py-3 text-center text-slate-600 font-medium">{record.type || record.status || '-'}</td>
