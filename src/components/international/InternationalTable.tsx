...
interface InternationalTableProps {
  records: any[];
  currentPage: number;
...
export function InternationalTable({
  records,
  currentPage,
  setCurrentPage,
  itemsPerPage,
        ...
        <table className="w-full border-collapse collection-table" style={{ fontSize: '16px' }}>
          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-semibold text-slate-500 whitespace-nowrap">Tên điều ước / Thỏa thuận / Dự án</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Đối tác</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((record, index) => (
              <tr key={record.id} className="hover:bg-slate-50 transition-all group">
                ...
                <td className="px-4 py-3 text-center text-slate-600 font-medium">{record.partner}</td>
