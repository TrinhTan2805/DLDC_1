...
import { ParentChildRecognitionRecord } from '../ParentChildRecognitionModal';

interface ParentChildTableProps {
  records: ParentChildRecognitionRecord[];
  currentPage: number;
  setCurrentPage: (page: number) => void;
  itemsPerPage: number;
  setItemsPerPage: (items: number) => void;
  totalRecords: number;
  onViewRecord: (record: ParentChildRecognitionRecord) => void;
        ...
        <table className="w-full border-collapse collection-table" style={{ fontSize: '16px' }}>
          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Họ và tên cha/mẹ</th>
              ...
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Họ và tên con</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Giới tính con</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Ngày sinh con</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((record, index) => (
              <tr key={record.id} className="hover:bg-slate-50 transition-all group">
                ...
                <td className="px-4 py-3 text-center font-semibold text-slate-900">{record.parentName}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium">{record.parentGender}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium font-mono">{record.parentBirthDate}</td>
                <td className="px-4 py-3 text-center font-semibold text-blue-700">{record.childName}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium">{record.childGender}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium font-mono">{record.childBirthDate}</td>
