...
import { Calendar, Download, HandHeart, UserMinus, Baby, Activity, UserCog, Accessibility, Heart } from 'lucide-react';
...
export function SocialSecurityGroupPage({ mode = 'thu thập', context = 'thu thập' }: SocialSecurityGroupPageProps) {
  ...
  if (mode === 'xử lý') {
    return <GenericProcessingPage systemName="BHXH và Giảm nghèo" datasets={stats.map((s, idx) => ({ id: s.id || `item_${idx}`, name: s.title }))} />;
  }

  const tableData: DatabaseRecord[] = [
    { name: 'Dữ liệu Hưởng trợ giúp XH', category: 'BHXH và Giảm nghèo', todayCount: 167000, errorCount: 32 },
    { name: 'Dữ liệu Thông tin người nghèo, cận nghèo', category: 'BHXH và Giảm nghèo', todayCount: 267000, errorCount: 45 },
    { name: 'Dữ liệu Người đơn thân', category: 'BHXH và Giảm nghèo', todayCount: 65000, errorCount: 18 },
    { name: 'Dữ liệu Trẻ em là đối tượng BTXH', category: 'BHXH và Giảm nghèo', todayCount: 91000, errorCount: 22 },
    { name: 'Dữ liệu Người có HIV', category: 'BHXH và Giảm nghèo', todayCount: 27000, errorCount: 12 },
    { name: 'Dữ liệu Người cao tuổi', category: 'BHXH và Giảm nghèo', todayCount: 203000, errorCount: 38 },
    { name: 'Dữ liệu Thông tin về người khuyết tật', category: 'BHXH và Giảm nghèo', todayCount: 134000, errorCount: 28 },
  ];

  // Chart data matching stats totals
  const chartData = stats.map((stat, index) => {
    const shortNames = ['Trợ giúp XH', 'Nghèo, cận nghèo', 'Người đơn thân', 'Trẻ em BTXH', 'Người HIV', 'Cao tuổi', 'Khuyết tật'];
    return {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Stats Grid - 7 cards in 4 columns */}
