...
import { Calendar, Download, UserCircle2, Flag, MapPin, Church, Landmark, Map, Heart, FileUser } from 'lucide-react';
...
export function CategoryGroupPage({ mode = 'thu thập', context = 'thu thập' }: CategoryGroupPageProps) {
  ...
  if (mode === 'xử lý') {
    return <GenericProcessingPage systemName="Danh mục" datasets={stats.map((s, idx) => ({ id: s.id || `item_${idx}`, name: s.title }))} />;
  }

  const tableData: DatabaseRecord[] = [
    { name: 'Dữ liệu Danh mục giới tính', category: 'Danh mục', todayCount: 15600, errorCount: 12 },
    { name: 'Dữ liệu Danh mục và mã các dân tộc', category: 'Danh mục', todayCount: 145000, errorCount: 28 },
    { name: 'Dữ liệu Danh mục và mã Quốc gia, Quốc tịch', category: 'Danh mục', todayCount: 112000, errorCount: 22 },
    { name: 'Dữ liệu Danh mục và mã các Tôn giáo', category: 'Danh mục', todayCount: 52000, errorCount: 15 },
    { name: 'Dữ liệu Danh mục cơ quan', category: 'Danh mục', todayCount: 278000, errorCount: 35 },
    { name: 'Dữ liệu Danh mục đơn vị hành chính', category: 'Danh mục', todayCount: 398000, errorCount: 42 },
    { name: 'Dữ liệu Danh mục và mã mối quan hệ trong gia đình', category: 'Danh mục', todayCount: 78000, errorCount: 18 },
    { name: 'Dữ liệu Danh mục mã giấy tờ tùy thân', category: 'Danh mục', todayCount: 182000, errorCount: 25 },
  ];

  // Chart data matching stats totals
  const chartData = stats.map((stat, index) => {
    const shortNames = ['Giới tính', 'Dân tộc', 'Quốc gia', 'Tôn giáo', 'Cơ quan', 'ĐV hành chính', 'Quan hệ GĐ', 'Giấy tờ'];
    return {
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6">
        ...
        <div className="relative h-96 pl-16 pr-4 pt-8 pb-20">
          ...
          <div className="absolute left-16 right-4 top-8 bottom-20 border-l border-b border-slate-300">
            {[0, 1000000, 2000000, 3000000, 4000000].map((value) => {
