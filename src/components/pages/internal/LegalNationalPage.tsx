...
export function LegalNationalPage({ mode = 'thu thập', context = 'thu thập' }: LegalNationalPageProps) {
  ...
  const tableData: DatabaseRecord[] = [
    ...
    { name: 'Dữ liệu Quan hệ giữa các điều khoản trong các văn bản quy phạm pháp luật', category: 'CSDL quốc gia về PL', todayCount: 20000, errorCount: 30 },
    { name: 'Dữ liệu Văn bản hợp nhất', category: 'CSDL quốc gia về PL', todayCount: 20000, errorCount: 30 },
    { name: 'Dữ liệu Hệ thống hóa văn bản quy phạm pháp luật', category: 'CSDL quốc gia về PL', todayCount: 20000, errorCount: 30 },
  ];

  // Chart data matching stats totals
  const chartData = stats.map((stat, index) => {
    const names = ['VBQPPL', 'Nội dung VBQPPL', 'Quan hệ điều khoản', 'VB hợp nhất', 'Hệ thống hóa'];
    return {
      name: names[index],
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Stats Grid - 5 cards for legal national database */}
