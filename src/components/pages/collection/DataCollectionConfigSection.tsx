    ...
    <div className="w-full max-w-4xl mx-auto px-6 py-4 space-y-6">
      ...
      <div className="flex flex-col gap-1">
        <div className="flex items-center gap-6">
          <label className="w-32 text-base font-medium shrink-0">Loại tần suất</label>
          <select 
            value={frequencyType}
            onChange={e => setFrequencyType(e.target.value)}
            className="flex-1 max-w-2xl px-4 py-2 text-base border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm"
          >
            <option value="Tạo mới">Tạo mới</option>
            <option value="Cập nhật">Cập nhật</option>
          </select>
        </div>
        <div className="ml-[152px] text-base text-slate-500 italic">
      ...
      <div className="flex items-center gap-2">
        <span className="text-base font-bold text-slate-800">Tần suất</span>
        <div className="flex-1 border-t border-slate-200 mt-1"></div>
      </div>

      {/* Lặp lại */}
      <div className="flex items-center gap-6">
        <label className="w-32 text-base font-medium shrink-0">Lặp lại</label>
        <select 
          value={repeatMode}
          onChange={e => {
          ...
          className="flex-1 max-w-2xl px-4 py-2 text-base border border-blue-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-blue-50/30 shadow-sm"
        >
      ...
      <div className="flex items-start gap-6">
        <label className="w-32 text-base text-slate-700 font-medium shrink-0 pt-2">Lặp lại trong</label>
        <div className="flex-1 max-w-2xl flex flex-col gap-3">
          <div className="flex items-center gap-3">
            <input 
              ...
              className="w-24 px-4 py-2 text-base border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm"
            />
            <span className="text-base text-slate-700">{getIntervalLabel()}</span>
          </div>
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-3 mt-1">
              {DAYS_OF_WEEK.map(day => (
                <label key={day.id} className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-base text-slate-700">{day.label}</span>
                </label>
              ))}
            </div>
          )}
          ...
          {repeatMode === 'Hằng tháng' && (
            <div className="flex flex-col gap-3 mt-1">
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                />
                <span className="text-base text-slate-700 whitespace-nowrap">ngày</span>
                <input 
                  type="number" 
                  min="1" max="31"
                  value={monthDate}
                  ...
                  className="w-20 px-3 py-1.5 text-base border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-slate-50 disabled:text-slate-400 shadow-sm"
                />
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <select 
                  ...
                  className="px-3 py-1.5 text-base border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-slate-50 disabled:text-slate-400 shadow-sm"
                >
      ...
      {repeatMode !== 'Hàng phút' && repeatMode !== 'Hàng giờ' && (
        <>
          <div className="flex items-center gap-2 pt-2">
            <span className="text-base font-bold text-slate-800">Thời gian</span>
            <div className="flex-1 border-t border-slate-200 mt-1"></div>
          </div>

          <div className="flex items-center gap-6">
            <label className="w-32 text-base text-slate-700 font-medium shrink-0">Thực hiện lúc</label>
            <div className="flex items-center max-w-[200px] border border-slate-300 rounded-lg overflow-hidden shadow-sm">
              <input 
                ...
                className="flex-1 px-4 py-2 text-base focus:outline-none bg-white"
              />
      ...
      {/* Divider Tóm lược */}
      <div className="flex items-center gap-2 pt-2">
        <span className="text-base font-bold text-slate-800">Tóm lược</span>
        <div className="flex-1 border-t border-slate-200 mt-1"></div>
      </div>

      <div className="flex items-start gap-6">
        <label className="w-32 text-base font-medium shrink-0 pt-2">Mô Tả</label>
        <textarea 
          value={description}
          onChange={e => {
          ...
          rows={3}
          className="flex-1 max-w-2xl px-4 py-3 text-base border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white resize-y shadow-sm"
        />
      </div>
