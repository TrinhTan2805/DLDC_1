    ...
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md flex items-center justify-center z-[100] p-4" style={{ fontFamily: 'Inter, system-ui, sans-serif', fontSize: '16px' }}>
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-6xl flex flex-col h-[90vh] border border-slate-200 overflow-hidden animate-in fade-in zoom-in duration-300">
            ...
            <div className="p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                  <span className="text-sm text-slate-500">Schema/Database</span>
                  <span className="text-sm font-semibold text-slate-800">{data.schema}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                  ...
                  >
                    <div className="flex items-center gap-3 overflow-hidden">
                      ...
                      <div className="text-left overflow-hidden">
                        <p className={`text-sm font-bold truncate ${selectedTable === table.name ? 'text-white' : 'text-slate-800'}`}>
                          {table.name}
                        </p>
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {(mockColumns[selectedTable] || []).map((col) => (
                        <tr key={col.name} className="hover:bg-slate-50/30 transition-colors">
                          <td className="py-4 px-6 text-sm font-bold text-slate-800">{col.name}</td>
                          ...
                          <td className="py-4 px-6 text-sm text-slate-500 text-center">{col.length}</td>
                          <td className="py-4 px-6 text-sm text-slate-600">{col.description}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="px-8 py-5 border-t border-slate-100 flex justify-end bg-slate-50/30">
          <button
            onClick={onClose}
            className="px-8 py-2.5 text-sm font-bold text-white bg-slate-800 rounded-2xl hover:bg-slate-900 transition-all shadow-lg shadow-slate-200"
          >
            Đóng cửa sổ
          </button>
        </div>
