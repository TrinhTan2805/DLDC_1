    ...
    <div style={{ zIndex: 999999 }} className="fixed inset-0 z-[999999] flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-50/50">
          <h2 className="text-base font-bold text-slate-850">{requestData ? 'Cập nhật yêu cầu kết xuất dữ liệu' : 'Tạo yêu cầu kết xuất dữ liệu'}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          <div>
            ...
            <input value={org} onChange={(e) => setOrg(e.target.value)} placeholder="Ví dụ: Sở Nội vụ Lạng Sơn" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-[13px]" />
          </div>

          <div>
            <label className="block text-[13px] font-medium text-slate-700 mb-1">Nội dung yêu cầu <span className="text-red-500">*</span></label>
            <input value={requestContent} onChange={(e) => setRequestContent(e.target.value)} placeholder="Nhập nội dung yêu cầu..." className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-[13px]" />
          </div>
          ...
          <div>
            ...
            <select value={dataType} onChange={(e) => setDataType(e.target.value)} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-[13px] cursor-pointer">
          ...
          <div>
            ...
            <select value={dataOwner} onChange={(e) => setDataOwner(e.target.value)} className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-[13px] cursor-pointer">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            <div>
              ...
              <input value={fromDate} onChange={(e) => setFromDate(e.target.value)} type="date" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-[13px] cursor-pointer" />
            </div>
            <div>
              <label className="block text-[13px] font-medium text-slate-700 mb-1">Đến ngày</label>
              <input value={toDate} onChange={(e) => setToDate(e.target.value)} type="date" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-[13px] cursor-pointer" />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-4 text-[13px] mt-1">
              <label className="flex items-center gap-2 cursor-pointer"><input type="radio" className="accent-blue-650 w-4 h-4" checked={format === 'excel'} onChange={() => setFormat('excel')} />Excel</label>
              <label className="flex items-center gap-2 cursor-pointer"><input type="radio" className="accent-blue-650 w-4 h-4" checked={format === 'csv'} onChange={() => setFormat('csv')} />CSV</label>
              <label className="flex items-center gap-2 cursor-pointer"><input type="radio" className="accent-blue-650 w-4 h-4" checked={format === 'json'} onChange={() => setFormat('json')} />JSON</label>
              <label className="flex items-center gap-2 cursor-pointer"><input type="radio" className="accent-blue-650 w-4 h-4" checked={format === 'xml'} onChange={() => setFormat('xml')} />XML</label>
            </div>
          </div>

          <div>
            <label className="block text-[13px] font-medium text-slate-700 mb-1">Mô tả</label>
            <textarea value={purpose} onChange={(e) => setPurpose(e.target.value)} rows={3} placeholder="Mục đích sử dụng dữ liệu" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-[13px]" />
          </div>
        </div>
