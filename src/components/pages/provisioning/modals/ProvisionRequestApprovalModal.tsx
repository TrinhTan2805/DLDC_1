      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg flex flex-col">
        ...
        <div className="p-6 space-y-6">
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-200">
            ...
            <p className="text-sm text-slate-600 mb-1"><span className="font-semibold text-slate-700">Loại dữ liệu:</span> {requestData?.dataType || 'Thống kê hộ tịch 2025'}</p>
            <p className="text-sm text-slate-600 mb-1"><span className="font-semibold text-slate-700">Mục đích:</span> {requestData?.purpose || 'Phục vụ báo cáo quý I năm 2026'}</p>
          </div>
          ...
          <div>
            ...
            <div className="flex space-x-4">
              <button
                ...
                className={`flex-1 flex flex-col items-center p-4 border rounded-xl transition-all ${
                  status === 'approve'
              ...
              >
                ...
                <span className="font-bold text-xs uppercase tracking-wider">Phê duyệt yêu cầu</span>
              </button>
              <button
                onClick={() => setStatus('reject')}
                className={`flex-1 flex flex-col items-center p-4 border rounded-xl transition-all ${
                  status === 'reject'
                    ? 'border-red-500 bg-red-50/50 text-red-800 ring-2 ring-red-500/20'
                    : 'border-slate-200 bg-white text-slate-500 hover:bg-slate-50'
                }`}
              >
                <XCircle className={`w-8 h-8 mb-2 ${status === 'reject' ? 'text-red-600' : 'text-slate-400'}`} />
                <span className="font-bold text-xs uppercase tracking-wider">Từ chối yêu cầu</span>
              </button>
            </div>
          </div>
        ...
        <div className="px-6 py-4 border-t border-slate-200 flex justify-end space-x-3 bg-slate-50 rounded-b-xl">
          ...
          <button aria-label="Xác nhận"
            onClick={() => {
              if (status === 'approve') {
                onApprove(requestData?.id || 'YC-2026-0429');
              } else {
                onReject(requestData?.id || 'YC-2026-0429', rejectReason);
              }
              onClose();
            }}
            className={`px-4 py-2 text-white rounded-lg transition-colors font-bold text-sm ${
              status === 'approve' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-red-600 hover:bg-red-700'
            }`}
          >
