      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {activeTab === 'danh_sach_tai_khoan' && (
          <button
            ...
            className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg flex items-center shadow-md font-bold transition-all hover:scale-[1.02] active:scale-[0.98] text-sm"
          >
            <Plus className="w-4 h-4 mr-2" />
            Tạo tài khoản mới
          </button>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="border-b border-slate-200 bg-slate-50/50">
          <nav className="flex space-x-6 px-6" aria-label="Tabs">
            ...
            <button
              onClick={() => { setActiveTab('danh_sach_tai_khoan'); setSearchTerm(''); }}
              className={`whitespace-nowrap py-4 px-1 border-b-2 font-bold text-sm flex items-center transition-colors ${activeTab === 'danh_sach_tai_khoan'
        ...
        <div className="p-6">
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50 text-slate-500 text-xs border-b border-slate-200 uppercase tracking-wider">
                    ...
                    <th className="py-3 px-4 font-bold text-center">Phiên bản</th>
                  ...
                  {filteredApis.map(api => (
                    <tr key={api.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <span className="font-mono text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-0.5 rounded border border-blue-200">{api.version}</span>
                      </td>
                ...
                <tbody className="text-sm divide-y divide-slate-100">
                  {filteredRecons.map(recon => (
                    <tr key={recon.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <span className="font-mono text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-0.5 rounded border border-blue-200">{recon.version || 'v1.0'}</span>
                      </td>
          ...
          {/* TAB 4: DANH SÁCH TÀI KHOẢN (Flat Table View) */}
          {activeTab === 'danh_sach_tai_khoan' && (
            <div className="animate-in fade-in duration-200">
              <div className="border border-slate-200 rounded-xl overflow-x-auto bg-white shadow-sm">
                <table className="w-full text-left border-collapse table-auto">
                  <thead>
                    <tr className="bg-slate-50 text-slate-500 text-xs border-b border-slate-200 uppercase tracking-wider whitespace-nowrap">
                      <th className="py-3.5 px-5 font-bold">Tài khoản (Username)</th>
                      <th className="py-3.5 px-5 font-bold">Đơn vị được cấp quyền</th>
                      <th className="py-3.5 px-5 font-bold">Client ID / App Key</th>
                      <th className="py-3.5 px-5 font-bold">API được phép gọi</th>
                      <th className="py-3.5 px-5 font-bold">Ngày tạo</th>
                    ...
                    {accounts.length > 0 ? (
                      accounts.map(acc => (
                        <tr key={acc.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="py-4 px-5 font-semibold text-slate-700">{acc.organization}</td>
                          <td className="py-4 px-5 font-mono text-xs text-slate-500">{acc.clientId}</td>
                          <td className="py-4 px-5 text-xs font-medium text-slate-600">
                            <span className="bg-slate-100 text-slate-700 px-2.5 py-1 rounded border border-slate-200 whitespace-nowrap">
                              {acc.apiName}
                            </span>
                          </td>
                          <td className="py-4 px-5 text-slate-500 font-mono text-xs">{formatDateTime(acc.createdAt)}</td>
                          <td className="py-4 px-5 whitespace-nowrap">
                            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold ${acc.status === 'Hoạt động' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-slate-100 text-slate-600 border border-slate-200'}`}>
                              {acc.status === 'Hoạt động' ? <Unlock className="w-3 h-3" /> : <Lock className="w-3 h-3" />}
                              {acc.status}
                            </span>
                          </td>
                          <td className="py-4 px-5 text-center">
                            <div className="flex items-center justify-center gap-1">
                              ...
                              <button
                                ...
                                className={`p-1.5 rounded transition-colors ${acc.status === 'Hoạt động' ? 'text-slate-400 hover:text-orange-600 hover:bg-orange-50' : 'text-slate-400 hover:text-green-600 hover:bg-green-50'}`}
