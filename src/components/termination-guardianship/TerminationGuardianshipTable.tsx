...
import { TerminationGuardianshipCertRecord } from '../TerminationGuardianshipCertModal';

interface TerminationGuardianshipTableProps {
  records: TerminationGuardianshipCertRecord[];
  currentPage: number;
  setCurrentPage: (page: number) => void;
  itemsPerPage: number;
  setItemsPerPage: (items: number) => void;
  totalRecords: number;
  onViewRecord: (record: TerminationGuardianshipCertRecord) => void;
}

export function TerminationGuardianshipTable({
  records,
  currentPage,
  setCurrentPage,
  itemsPerPage,
  ...
  totalRecords,
  onViewRecord
}: TerminationGuardianshipTableProps) {
        ...
        <table className="w-full border-collapse collection-table" style={{ fontSize: '16px' }}>
          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Lý do chấm dứt</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Ngày chấm dứt</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((record, index) => (
              <tr key={record.id} className="hover:bg-slate-50 transition-all group">
                ...
                <td className="px-4 py-3 text-center text-slate-600 font-medium">{record.terminationReason}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium font-mono">{record.terminationDate}</td>
