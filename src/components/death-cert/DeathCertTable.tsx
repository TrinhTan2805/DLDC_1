...
interface DeathCertTableProps {
  records: DeathCertRecord[];
  currentPage: number;
  setCurrentPage: (page: number) => void;
  itemsPerPage: number;
  setItemsPerPage: (items: number) => void;
  totalRecords: number;
  onViewRecord: (record: DeathCertRecord) => void;
  onViewPdf: (url: string) => void;
}

export function DeathCertTable({
  records,
  currentPage,
  setCurrentPage,
  itemsPerPage,
        ...
        <table className="w-full border-collapse collection-table" style={{ fontSize: '16px' }}>
          <thead className="bg-slate-50 border-b border-slate-200 sticky top-0 z-10">
            <tr>
              ...
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Số giấy báo tử</th>
              <th className="px-4 py-3 text-center font-semibold text-slate-500 whitespace-nowrap">Họ và tên</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((record, index) => (
              <tr key={record.id} className="hover:bg-slate-50 transition-all group">
                ...
                <td className="px-4 py-3 text-center font-semibold text-slate-900 font-mono">{record.deathNoticeNumber}</td>
                <td className="px-4 py-3 text-center font-semibold text-slate-900">{record.deceasedName}</td>
                <td className="px-4 py-3 text-center text-slate-600 font-medium">{record.deceasedGender}</td>
