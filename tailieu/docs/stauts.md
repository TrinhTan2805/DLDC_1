...
- `[x]` Hộ tịch & Dân chính (`external/CivilRegistryDatabasePage.tsx`)
...
- `[x]` Người nhiễm HIV (`external/HIVPersonPage.tsx`)
- `[x]` Liệt sĩ (`external/MartyrRecordPage.tsx`)
- `[x]` Người có công (`external/MeritoriousPersonPage.tsx`)
- `[x]` Thân nhân người có công (`external/MeritoriousRelativePage.tsx`)
- `[x]` Hộ nghèo (`external/PovertyInfoPage.tsx`)
- `[x]` Trợ cấp xã hội (`external/SocialAssistancePage.tsx`)
- `[x]` Đơn thân (`external/SinglePersonPage.tsx`)
- `[x]` Trẻ em được bảo vệ (`external/ChildrenSocialProtectionPage.tsx`)
- `[x]` Thông tin trẻ em (`external/ChildrenInfoPage.tsx`)
- `[x]` Giấy tờ tùy thân (`external/IdentityDocumentPage.tsx`)
- `[x]` Đơn vị hành chính (`external/AdministrativeUnitPage.tsx`)
- `[x]` Danh mục cơ quan (`external/AgencyCategoryPage.tsx`)
- `[x]` Dân tộc (`external/EthnicCategoryPage.tsx`)
- `[x]` Giới tính (`external/GenderCategoryPage.tsx`)
- `[x]` Tôn giáo (`external/ReligionCategoryPage.tsx`)
- `[x]` Quan hệ gia đình (`external/FamilyRelationshipPage.tsx`)
- `[x]` Quốc tịch / Quốc gia (`external/CountryNationalityPage.tsx`)

---

## 7. PHÂN HỆ DỮ LIỆU NGHIỆP VỤ NỘI BỘ BTP

- `[x]` Luật sư & Trung tâm tư vấn pháp lý (`internal/LegalCenterPage.tsx`)
- `[x]` Hộ tịch nội bộ (`internal/CivilRegistryPage.tsx`)
- `[x]` Công chứng (`internal/NotarySystemPage.tsx`)
- `[x]` Hợp đồng công chứng (`internal/NotaryContractPage.tsx`)
- `[x]` Văn bản công chứng (`internal/NotaryDocumentPage.tsx`)
- `[x]` Đấu giá tài sản (`internal/AuctionPage.tsx`)
- `[x]` Biện pháp bảo đảm thi hành án (`internal/SecurityMeasuresPage.tsx`)
- `[x]` Trợ giúp pháp lý (`internal/LegalAidSystemPage.tsx`)
- `[x]` Vụ việc trợ giúp pháp lý (`internal/LegalAidCasePage.tsx`)
- `[x]` Người được trợ giúp pháp lý (`internal/LegalAidPersonPage.tsx`)
- `[x]` Quốc tịch (`internal/NationalitySystemPage.tsx`)
- `[x]` Hợp tác quốc tế (`internal/InternationalPage.tsx`)
- `[x]` Pháp luật quốc gia (`internal/LegalNationalPage.tsx`)
- `[x]` Hệ thống văn bản pháp luật (`internal/LegalDocumentSystemPage.tsx`)
- `[x]` Văn bản pháp luật BTP (`internal/MojDocPage.tsx`)
- `[x]` Văn bản Quốc hội (`internal/NationalAssemblyDocPage.tsx`)
- `[x]` Văn bản Nhà nước (`internal/GovernmentDocPage.tsx`)
- `[x]` Thống kê chuyên ngành (`internal/StatisticsCollectionPage.tsx`)
- `[x]` Phán quyết dân sự (`internal/CivilJudgmentPage.tsx`)
- `[x]` Thông tin pháp lý dân sự (`internal/CivilLegalInfoPage.tsx`)
- `[x]` Trung tâm pháp lý dân sự (`internal/CivilLegalCenterPage.tsx`)
- `[x]` Đăng ký kinh doanh (`internal/BusinessRegistryPage.tsx`)
- `[x]` Đăng ký thành lập (`internal/BusinessRegistrationPage.tsx`)
- `[x]` Thông tin doanh nghiệp (`internal/BusinessInfoPage.tsx`)
- `[x]` Hồ sơ gia đình (`internal/FamilyBasePage.tsx`)
- `[x]` Quản lý vụ án (`internal/CaseManagementPage.tsx`)
- `[x]` Bồi thường nhà nước (`internal/StateCompensationPage.tsx`)
- `[x]` Giấy khai sinh (`internal/BirthCertificatePage.tsx`)
- `[x]` Giấy đăng ký kết hôn (`internal/MarriageCertificatePage.tsx`)
- `[x]` Giấy ly hôn (`internal/DivorceCertificatePage.tsx`)
- `[x]` Giấy khai tử (`internal/DeathCertificatePage.tsx`)
- `[x]` Giấy nhận nuôi con nuôi (`internal/AdoptionCertificatePage.tsx`)
- `[x]` Giấy giám hộ (`internal/GuardianshipCertificatePage.tsx`)
...
### Trang chính
- `[x]` Dashboard Cung cấp dữ liệu (`provisioning/DataProvisionDashboard.tsx`)
- `[x]` Quản lý API cung cấp dữ liệu (`provisioning/DataProvisionApiManagementPage.tsx`)
- `[x]` Thiết lập dịch vụ cung cấp (`provisioning/DataProvisionServiceSetupPage.tsx`)
- `[x]` Danh sách dịch vụ cung cấp (`provisioning/DataProvisionServicesPage.tsx`)
- `[x]` Giám sát cung cấp dữ liệu (`provisioning/DataProvisionMonitoringPage.tsx`)
- `[x]` Yêu cầu sử dụng dữ liệu (`provisioning/DataProvisionRequestPage.tsx`)
- `[x]` Xem tài liệu API (`provisioning/PreviewApiDocsPage.tsx`)
- `[x]` Đối soát cung cấp (`provisioning/DataReconciliationPage.tsx`)

### Components & Tabs
- `[x]` Bảng dữ liệu dịch vụ (`provisioning/components/ServiceDataTable.tsx`)
- `[x]` Tab Tài liệu API (`provisioning/tabs/ApiDocumentationTab.tsx`)
- `[x]` Tab Nhật ký kiểm toán (`provisioning/tabs/AuditLogsTab.tsx`)
...
- `[x]` Modal Yêu cầu dữ liệu (`provisioning/modals/ProvisionDataRequestModal.tsx`)
- `[x]` Modal Phê duyệt yêu cầu (`provisioning/modals/ProvisionRequestApprovalModal.tsx`)
- `[x]` Modal Xuất khẩu yêu cầu (`provisioning/modals/ProvisionRequestExportModal.tsx`)
- `[x]` Modal Bàn giao yêu cầu (`provisioning/modals/ProvisionRequestHandoverModal.tsx`)
- `[x]` Modal Chi tiết bàn giao (`provisioning/modals/ProvisionHandoverDetailModal.tsx`)
- `[x]` Modal Kiểm soát truy cập (`provisioning/modals/ProvisionAccessControlModal.tsx`)
- `[x]` Modal Đối soát API (`provisioning/modals/ProvisionReconciliationApiModal.tsx`)
- `[x]` Modal Chi tiết đối soát (`provisioning/modals/ProvisionReconciliationDetailsModal.tsx`)
- `[x]` Modal Gửi phê duyệt (`provisioning/modals/SubmitApprovalModal.tsx`)
- `[x]` Modal Xuất báo cáo (`provisioning/modals/ProvisionExportReportModal.tsx`)
- `[x]` Modal Kiểm soát truy cập (`provisioning/modals/AccessControlModal.tsx`)
- `[x]` Modal Chọn API (`provisioning/modals/ApiSelectionModal.tsx`)
- `[x]` Modal So sánh phiên bản API (`provisioning/modals/ApiVersionCompareModal.tsx`)
- `[x]` Modal Trường tính toán (`provisioning/modals/CalculatedFieldModal.tsx`)
- `[x]` Modal Thiết kế gói dữ liệu (`provisioning/modals/PacketDesignModal.tsx`)
- `[x]` Modal Cấu hình trường chia sẻ (`provisioning/modals/SharedFieldsConfigModal.tsx`)
- `[x]` Modal Chi tiết bản ghi (`provisioning/modals/RecordDetailModal.tsx`)
