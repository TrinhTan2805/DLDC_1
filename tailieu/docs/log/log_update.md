...
2. **Đồng bộ hóa giao diện trang Đối soát cung cấp (Quy trình đối soát dữ liệu):**
   ...
   - **Tác vụ & Header**: Thay thế nút hành động xem chi tiết cũ bằng icon con mắt màu đen (`Eye`) không viền, không nền. Loại bỏ 2 nút tác vụ *"Cấu hình"* và *"Đối soát ngay"* ở phía trên góc phải của header chi tiết và tăng cỡ chữ tiêu đề *"Lịch sử đối soát"* lên `18px`.
   ...
   - **Modal chi tiết**: Cấu hình lại độ mờ nền của modal chi tiết đối soát thành `bg-black/50 backdrop-blur-sm z-[999999]` (style inline zIndex `999999`) và đồng bộ font chữ nội dung về `13px` và tiêu đề `18px`.
